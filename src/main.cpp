...
#if defined(NDEBUG)
# error "Maylolcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa49f4764d2d5494fc653370d2de5e9943249d2a3af6cab8bf2e9b0d39f41bde4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Maylolcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Maylolcoin Signed Message:\n";
...
{
    // Maylolcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1) return 99900000 * COIN;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 600); // Maylolcoin: 600 blocks 

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 24 * 60 * 60;
static const int64 nTargetSpacing = 2 * 60; // Maylolcoin: 120 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Maylolcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool SetBestChain(CValidationState &state, CBlockIndex* pindexNew)
{
    ...
    if ((pindexNew->nHeight % 1) == 0 || (!fIsInitialDownload && (pindexNew->nHeight % 144) == 0))
    {
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Maylolcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is Maylolcoin !";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1512534058;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2125646;
        ...
        assert(block.hashMerkleRoot == uint256("0x8ea3e58a2a57ef47fb4d191a4a4783613530ffee791b153f2207c42899776488"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Maylolcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MaylolcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MaylolcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MaylolcoinMiner(CWallet *pwallet)
{
    printf("MaylolcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("maylolcoin-miner");
    ...
    try { loop {
        ...
        printf("Running MaylolcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MaylolcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MaylolcoinMiner, pwallet));
}
